            ...
            <div className="flex flex-row overflow-x-auto pb-3 sm:pb-4 gap-3 sm:gap-6 no-scrollbar">
              {allCategories.map((category) => (
                <div key={category.id} className="flex flex-col items-start min-w-[80px] max-w-[80px] sm:min-w-[100px] sm:max-w-[100px] flex-shrink-0">
                   ...
                   <Link
                     href={`/${category.slug}`}
                     className="whitespace-nowrap px-3 py-1.5 sm:px-4 sm:py-2 rounded-md text-xs sm:text-sm font-bold text-black hover:bg-gray-100 transition-colors"
                    ...
                    {category.id === "schedule" ? (
                      <>
                         <Link
                           ...
                           className="px-2 py-1.5 sm:px-3 sm:py-2 text-xs sm:text-sm text-gray-700 hover:bg-gray-100 rounded-md transition-colors whitespace-nowrap"
                           onClick={handleLinkClick}
                         >
                          연간일정
                        </Link>
