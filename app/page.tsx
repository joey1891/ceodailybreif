'use client'; // 💡 Vercel 에러 해결: 클라이언트 컴포넌트 명시

import { useEffect, useState } from 'react';
import { supabase } from '@/utils/supabase'; // 실제 DB 연결
  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      const [
        ...
        supabase.from('articles').select('*').eq('is_published', true).order('view_count', { ascending: false }).limit(6), // MOST VIEWED 6개
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 sm:py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10 pb-8 sm:pb-12 items-start">
          ...
          <div className="lg:col-span-8 flex flex-col gap-8 sm:gap-10">
            {headlines.MAIN_HERO ? (
              <Link href={`/article?id=${headlines.MAIN_HERO.id}`} className="block">
                  ...
                  {headlines.MAIN_HERO.image_url && (
                    <div className="w-full bg-gray-100 mb-4 sm:mb-6 overflow-hidden rounded">
                      <img 
                        ...
                        alt="Lead story" 
                        // 세로 이미지가 길어질 경우의 디자인 안정성을 위한 처리
                        className="w-full h-auto object-cover group-hover:scale-[1.02] transition-transform duration-700 ease-in-out grayscale-[20%]"
                      />
                    </div>
                  )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 mt-2 sm:mt-0">
              {[
                ...
                subArticle ? (
                  <Link key={idx} href={`/article?id=${subArticle.id}`} className="block">
          ...
          <div className="lg:col-span-4 flex flex-col gap-8 sm:gap-10">
            ...
            {}
            {/* 2. ADVERTISEMENT BANNER Section */}
            <div className="px-2 sm:px-0 flex justify-center w-full my-2">
